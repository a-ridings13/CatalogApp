...
def gconnect():
    ...
    if result['issued_to'] != CLIENT_ID:
        response = make_response(json.dumps("Token's client ID does not match this application's ID."), 401)
    ...
    if stored_creds is not None and g_id == stored_g_id:
        response = make_response(json.dumps('Current user already signed in!'), 200)
        response.headers['Content-Type'] = 'application/json'
...
def catalog():
    ...
    items = conn.query(Category_Item).order_by(Category_Item.id.desc()).limit(5)
...
def addItem(category_id):
    ...
    if request.method == 'POST':
        newItem = Category_Item(name=request.form['name'], description=request.form['desc'],
        ...
        return render_template('addItem.html', category_id=category_id, cat=cat)
...
def editItem(category_id, item_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editItem.html', item=itemUpdate, category_id=category_id, item_id=item_id)
...
def categoryItemsJSON(category_id):
    ...
    items = conn.query(Category_Item).filter_by(category_id=category_id).all()
    return jsonify(CategoryName=category.serialize, CategoryItems=[i.serialize for i in items])
